"use client";

import { toast } from 'react-toastify'
import { useParams,useRouter } from "next/navigation";
import React, { useEffect, useState } from "react";

interface TimeSlot {
  datetime: Date;
  time: string;
}

const Page = () => {
  const { doctorId,patientId } = useParams();
  const [slotIndex, setSlotIndex] = useState(0);
  const [slotTime, setSlotTime] = useState("");
  const [docInfo, setDocInfo] = useState<any>(null);
  const [docSlots, setDocSlots] = useState<TimeSlot[][]>([]);
  const daysOfWeek = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

  // Fetch doctor information
  const fetchDocInfo = async () => {
    try {
      const res = await fetch(
        `http://localhost:3000/api/v1/doctor/getById/${doctorId}`
      );
      if (!res.ok) throw new Error("Failed to fetch doctor info");

      const data = await res.json();
      setDocInfo(data);
    } catch (error) {
      console.error("Error fetching doctor info:", error);
    }
  };

   const route = useRouter();
  console.log(patientId)
  // Generate available slots for the next 7 days
  const getAvailableSlots = () => {
    const slots: TimeSlot[][] = [];
    const today = new Date();

    for (let i = 0; i < 7; i++) {
      const currentDate = new Date(today);
      currentDate.setDate(today.getDate() + i);
      const endTime = new Date(currentDate);
      endTime.setHours(21, 0, 0, 0);

      if (i === 0) {
        currentDate.setHours(
          currentDate.getHours() > 10 ? currentDate.getHours() + 1 : 10
        );
        currentDate.setMinutes(currentDate.getMinutes() > 30 ? 30 : 0);
      } else {
        currentDate.setHours(10, 0, 0, 0);
      }

      const timeSlots: TimeSlot[] = [];

      while (currentDate < endTime) {
        timeSlots.push({
          datetime: new Date(currentDate),
          time: currentDate.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
        });

        currentDate.setMinutes(currentDate.getMinutes() + 30);
      }

      slots.push(timeSlots);
    }

    setDocSlots(slots);
  };

  const bookAppointment = async () => {
    try {
      if (!docSlots || !docSlots[slotIndex] || !docSlots[slotIndex][0]) {
        toast.error("Invalid slot selection");
        return;
      }
  
      const date = new Date(docSlots[slotIndex][0].datetime);
  
      if (isNaN(date.getTime())) {
        toast.error("Invalid date format");
        return;
      }
  
      let day = date.getDate();
      let month = date.getMonth() + 1;
      let year = date.getFullYear();
      const slotDate = `${day}_${month}_${year}`;
  
      // Ensure required fields are defined
      if (!slotTime || !doctorId || !patientId) {
        toast.error("Missing required fields: slotTime, doctorId, or patientId");
        return;
      }
  
      const res = await fetch("http://localhost:3000/api/v1/appointment/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slotDate, slotTime, doctorId, patientId }),
        credentials: "include", // Required for cookies/authentication
      });
  
      const data = await res.json();
  
      if (data.success) {
        toast.success(data.message);
        fetchDocInfo();
        route.push("/my-appointment");
      } else {
        toast.error(data.message);
      }
    } catch (error) {
      console.error("Error booking appointment:", error);
      toast.error("An error occurred. Please try again.");
    }
  };
  

  useEffect(() => {
    fetchDocInfo();
  }, [doctorId]);

  useEffect(() => {
    getAvailableSlots();
  }, []);

  return (
    docInfo && (
      <div className="text-gray-200 bg-gray-900 min-h-screen p-6">
        <div className='flex flex-col md:flex-row gap-4'>
          <div>
            <img className='bg-gray-800 w-full sm:max-w-72 rounded-lg'  src={'data:image/png;base64,'}  alt="" />
          </div>

          <div className='flex-1 border border-gray-700 rounded-lg p-8 py-7 bg-gray-800 mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
            <p className='flex items-center gap-2 text-2xl font-medium text-gray-100'>{docInfo.name}</p>
            <div className='flex items-center gap-2 text-sm mt-1 text-gray-400'>
              <p>{docInfo.degree} - {docInfo.specialization}</p>
              <button className='py-0.5 px-2 border text-xs rounded-full border-gray-600'>{docInfo.experience}</button>
            </div>

            <div>
              <p className='flex items-center gap-1 text-sm font-medium text-gray-300 mt-3'>About</p>
              <p className='text-sm text-white max-w-[700px] mt-1'>{docInfo.specialization}</p>
            </div>
            <p className='text-gray-400 font-medium mt-4'>
              Appointment fee: <span className='text-gray-300'>{docInfo.fees}</span>
            </p>
          </div>
        </div>

        <p className="text-lg font-semibold text-gray-300 mt-6">Booking Slots</p>

        {/* Date Selection */}
        <div className="flex gap-3 items-center w-full overflow-x-auto scrollbar-hide mt-4">
          {docSlots.map((slots, index) => (
            <div
              key={index}
              onClick={() => setSlotIndex(index)}
              className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${
                slotIndex === index
                  ? "bg-sky-600 text-white"
                  : "border border-gray-600 text-gray-300"
              }`}
            >
              <p>{slots[0] && daysOfWeek[slots[0].datetime.getDay()]}</p>
              <p>{slots[0] && slots[0].datetime.getDate()}</p>
            </div>
          ))}
        </div>

        {/* Time Selection */}
        <div className="flex gap-3 items-center w-full overflow-x-auto scrollbar-hide mt-4">
          {docSlots[slotIndex]?.map((slot, index) => (
            <p
              key={index}
              onClick={() => setSlotTime(slot.time)}
              className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${
                slot.time === slotTime
                  ? "bg-sky-600 text-white"
                  : "border border-gray-600 text-gray-300"
              }`}
            >
              {slot.time.toLowerCase()}
            </p>
          ))}
        </div>

        <button onClick={bookAppointment} className="bg-sky-600 text-white text-sm font-light px-14 py-3 rounded-full my-6 hover:bg-sky-500">Book an Appointment</button>
      </div>
    )
  );
};

export default Page;
